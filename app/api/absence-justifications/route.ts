...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const saved = await AbsenceJustification.findOneAndUpdate(
      { studentId, date },
    ...
    console.error('POST /api/absence-justifications failed:', error);
    return NextResponse.json(
      { error: 'Falha ao salvar justificativa de ausência.' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const platformConn = await connectPlatformDB();
    const AbsenceJustification = getAbsenceJustificationModel(platformConn);
    await AbsenceJustification.deleteOne({ studentId, date });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('DELETE /api/absence-justifications failed:', error);
    return NextResponse.json(
      { error: 'Falha ao apagar justificativa de ausência.' },
      { status: 500 }
    );
  }
}
